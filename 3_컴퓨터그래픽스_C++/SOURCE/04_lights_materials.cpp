...
void init()
{
	...
	cout << "Keyboard input: 4 for a planes with many small squres" << endl;
}
...
{
	// Material
	GLfloat mat_ambient[4] = { 0.33, 0.22, 0.33, 1 };
	GLfloat mat_diffuse[4] = { 0.78, 0.57, 0.11,1 };
	GLfloat mat_specular[4] = { 0.99, 0.91, 0.81, 1 };
...
void keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		case GLFW_KEY_I:	reinitialize(); break;
			...
			// Play on / off
		case GLFW_KEY_V:	pause = !pause; break;
